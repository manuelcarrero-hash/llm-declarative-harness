...
| `REVIEW_01` | Review | Yes for every deliverable; independence when material change occurred | Before delivery or persistence, were invented decisions, confusions, unsupported premises, compliance and harness exposure reviewed; and for material changes, did an independent reviewer inspect the actual work? | Proportional checklist, sources, diff, findings, corrections, rerun and verdict |
...
| `STATE_01` | State | Yes | Did authoritative state distinguish current facts, background, materials, style, proposals and assumptions; preserve provenance; and represent validation and telemetry honestly? | Snapshot, provenance, confirmations, corrections and pulse compared with direct sources |
...
- `REVIEW_01` fails when an outcome is delivered or persisted without checking all five pre-delivery categories. If a material change occurred, it also cannot be marked `NOT_APPLICABLE` merely because no independent reviewer was assigned.
...
- `STATE_01` fails when prior memory, a reusable case, a style reference, a proposal or an agent interpretation is stored as confirmed fact without provenance and confirmation; correcting the deliverable without repairing state does not cure the failure.
