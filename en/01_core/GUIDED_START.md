...
5. Consult `../04_adapters/COMPATIBILITY_MATRIX.md` only as guidance and declare capabilities with the corresponding profile. Separate status, evidence, authorization and freshness for every capability. Current evidence prevails over the matrix; a current contradiction corrects the profile and flags the guidance for review.
...
- Do not ask the person to complete the compatibility matrix. Differences among products, interfaces, plans or versions from the same provider must be checked, not presumed.
